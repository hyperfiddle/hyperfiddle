(ns hyperfiddle.ui.typeahead2 ; TODO implement with TablePicker and move to electric-forms3
...
(e/defn ComboboxWrapper [initial-state format-value Body]
  (binding [!state (atom {::open false, ::search "", ::value (e/snapshot initial-state)})] ; prevent mount/unmount
    (binding [state (e/watch !state)
              ...
              hyperfiddle.ui.typeahead2/format-value format-value]
  ...
  (let [selected? (= value (::value state))]
    (dom/div
      ...
      (dom/On "click" (fn [^js e]
                        ...
                        (set! (.-hyperfiddle_ui_typeahead2_submit e) true)
                        e) nil)
                  ...
                  (e/for [opt options]
                    (ComboboxOption opt
                      (e/fn [label selected?]
                        (dom/span
                          (dom/text label))))))))))
        (let [e (e/Filter some? (dom/On "click" (fn [^js e] (when (.-hyperfiddle_ui_typeahead2_submit e) e)) nil))]
          (if (= value selected)
            (e/amb)
            (let [[t err] (e/Token e)
                  waiting? (some? t)
              ...
              (if waiting?
                (do (dom/props input-node {:aria-busy true})
                    {:hyperfiddle.electric-forms4/token (fn ([] (t) (reset-typeahead! !state input-node)) ; success: Typeahead's authoritative value is expected to loop back from server
                               ([err] (t err) (reset-typeahead! !state input-node)))
                     :hyperfiddle.electric-forms4/name name
