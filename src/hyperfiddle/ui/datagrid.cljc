(ns hyperfiddle.ui.datagrid
  ...
  #?(:cljs (:require-macros [hyperfiddle.ui.datagrid]))
  ...
  (e/client
    (let [!loading? (atom false)]
      (binding [loading? (e/watch !loading?)
                ...
                hyperfiddle.ui.datagrid/row-height row-height]
          ...
          (binding [columns (ordered-columns (vals columns-index))]
            (dom/table
              ...
              (dom/props {:id id
                          ...
                          :class "datagrid grid content-start",
  ...
  (e/client
    (dom/th
      (let [{::keys [id] :as column} (make-column dom/node frozen?)]
        (dom/props {:class "sticky top-0 block truncate"})
...
(e/defn* Row [Body]
  (e/client
    (dom/tr
      (dom/props {:class (subgrid? "hyperfiddle subgrid-row" "contents")})
      (Body.))))
